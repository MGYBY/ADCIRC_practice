...
def plot_nc(nc, var, levels, ncvec = None, dxvec = None, dyvec = None, 
           ...
           background_map = False, extend = 'neither', sf = False, sf_path = ''):
    ...
    if background_map == True:
        ...
        ax.add_feature(cfeature.LAND, edgecolor='face', facecolor='#C0C0C0')
        ax.add_feature(cfeature.COASTLINE,lw=0.25)
        # ax.add_feature(cfeature.LAKES)

    if sf == True:
        # shp_file = gpd.read_file(sf_path)
        shape_feature = ShapelyFeature(Reader(sf_path).geometries(), ccrs.PlateCarree(), facecolor='none')
