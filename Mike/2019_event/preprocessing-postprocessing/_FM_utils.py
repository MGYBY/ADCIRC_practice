...
def __add_colorbar(
    ...
    cax = make_axes_locatable(ax).append_axes("bottom", size="10%", pad=0.7)
...
def _get_node_centered_data(
    ...
    Parameters
    ----------
    node_coordinates: np.array(float)
        node coordinates
    element_table: np.array[int]
        element table
    element_coordinates: np.array(float)
        element coordinates
    data : np.array(float)
        cell-centered data
