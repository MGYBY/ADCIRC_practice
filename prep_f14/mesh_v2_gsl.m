...
fh1 = edgefx('geodata', gdat, 'fs', R_val, 'max_el', [(max_el) -10 0.0; (max_el)/1.5 -20 -10.0; (max_el)/3.0 -700 -20], 'dt', dt_val, 'g', grade_val, 'slp',slp);

mshopts = meshgen('ef',fh1,'bou',gdat,'plot_on',1,'nscreen',5,'proj','trans', 'itmax', 211, 'qual_tol', 0.020);
...
m = interp(m,gdat, 'type', 'depth' ,'mindepth', 0.20); % interpolate bathy to the mesh with minimum depth of 1 m
% m = interp(m,gdat);
